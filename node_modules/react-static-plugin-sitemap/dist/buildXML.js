...
function generateXML(state) {
  ...
  .map(function (route) {
    ...
    return ['<url>', xmlArrayOutput(attributesArr, staging), '</url>'].join(staging ? '\n' : '');
  }).join(staging ? '\n' : '');
  return ["<?xml version=\"1.0\" encoding=\"UTF-8\"?>", "<urlset xmlns=\"http://www.sitemaps.org/schemas/sitemap/0.9\" xmlns:image=\"http://www.google.com/schemas/sitemap-image/1.1\">", xmlRoutes, "</urlset>"].join(staging ? '\n' : '');
}
...
function xmlArrayOutput(values, staging) {
  return _toConsumableArray(values.map(function (_ref3) {
    var key = _ref3.key,
        value = _ref3.value;
